    ...
    if (m_toggle) {
        for (char i = 0; i < MAX_ROWS; i++) {
            for (char j = 0; j < MAX_COLS; j++) {
                ...
                //if m_otherWorld is alive, and wasn't in last step, update m_stats
                if ((m_otherWorld[i][j] != m_world[i][j]) && (m_otherWorld[i][j] == ALIVE))
                    stats().record(i, j, 1);
            }
        }
        m_toggle = !m_toggle;
        ...
        for (char i = 0; i < MAX_ROWS; i++) {
            for (char j = 0; j < MAX_COLS; j++) {
                ...
                //if m_world is alive, and wasn't in last step, update m_stats
                if ((m_world[i][j] != m_otherWorld[i][j]) && (m_world[i][j] == ALIVE))
                    stats().record(i, j, 1);
            }
        }
        m_toggle = !m_toggle;
